...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "TP02 - Rendu 3D", nullptr, nullptr);
    ...
    GLShader shader;
    shader.LoadVertexShader("shaders/cube.vs.glsl");
    shader.LoadFragmentShader("shaders/cube.fs.glsl");
    if (!shader.Create()) return -1;
    uint32_t program = shader.GetProgram();

    // cube vertices from TP02 (positions only), here we add a simple per-vertex color
    ...
    int locMVP = glGetUniformLocation(program, "u_MVP");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        Mat4 V = Translation({ 0.f, 0.f, -6.f });
        float t = (float)glfwGetTime();
        Mat4 M = RotationY(t * 0.8f);
        ...
        {
            SaveScreenshotPPM("screens/tp02", w, h);
            captured = true;
